...
const PATIENT_STATUS = {
  예약접수: {
    ...
    icon: Calendar,
    description: "초기 예약 상태",
  },
  확정: {
    color: "bg-green-100 text-green-800",
    icon: CheckCircle,
    description: "예약이 확정된 상태",
  },
  대기: {
    color: "bg-yellow-100 text-yellow-800",
    icon: Clock,
    description: "대기실에서 대기 중",
  },
  진찰중: {
    color: "bg-purple-100 text-purple-800",
    icon: Activity,
    description: "의사가 진료 중",
  },
  치료중: {
...
export default function AdminDashboardScheduleAppointment({
  ...
  // 노쇼 통계 계산
  const noShowCount = safeAppointments.filter(
    (apt) => apt.status === "노쇼",
  ).length;
  const noShowRate =
    safeAppointments.length > 0
      ? ((noShowCount / safeAppointments.length) * 100).toFixed(
          1,
        )
      : "0";

  // 상태별 통계
  const statusStats = Object.keys(PATIENT_STATUS).reduce(
    (acc, status) => {
      acc[status] = safeAppointments.filter(
            ...
            <div className="space-y-2">
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min(parseFloat(noShowRate), 100)}%`,
                  }}
                />
              </div>
            </div>
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="flex items-center space-x-2">
                          {(() => {
                            const status =
                              appointment.status as PatientStatus;
                            const statusInfo =
